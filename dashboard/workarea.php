...
<?php 
	...
	// $count = 1;
	// while($count < 33){
	// 	$name = "workdata" . $count . "_1";
	// 	$$name = "";
	// }
	$count = 1;
	while( $row = $s->fetch()){
		// print_r($row);
		// $work_schedules[] = array($key1 => $row['timedate'],$key2 => $row['starttime'],$key3 => $row['endtime']);
		$name = "workdata" . $count . "_1";
		// $$name = $work_schedules[$count]['timedate1']."T". $work_schedules[$count]['starttime1'];
		$$name = $row['timedate']."T". $row['starttime'];
		$count = $count+1;
	}
